...
for epoch in range(EPOCHS):
  ...
  for (batch, (inp, tar)) in enumerate(train_batches):
    ...
    with open('./train_stats.txt', 'a+') as f:
        f.write(f'MHA {Stats.mha_time:.4f} MHA-Enc {Stats.mha_enc_time} MHA-Causal {Stats.mha_causal_time} MHA-Enc-Dec {Stats.mha_enc_dec_time} FFN {Stats.ffn_time:.4f} Downsampling {Stats.downsampling_time:.4f} Kernel-Transformation {Stats.transformation_time:.4f}\n')
