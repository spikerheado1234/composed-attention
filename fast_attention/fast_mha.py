...
def favor_attention(query,
  ...
  Stats.q_k_v_product += (qkv_product_end - qkv_product_start).numpy()
...
class Attention(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    ## It seems that the above was the reason of a parameter difference, so we stick to the canonical way of doing things. ##
  ...
  def call(self,
    ...
    ## These are the new ways we will map over the values. ##
    ...
    Stats.linear_transformation += (linear_trfm_end - linear_trfm_start).numpy()
    ...
    Stats.favour_time += (favour_attn_end - favour_attn_start).numpy()
